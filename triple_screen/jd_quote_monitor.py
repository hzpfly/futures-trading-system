...
# -*- coding: utf-8 -*-
"""
鸡蛋期货行情监控 - TQSDK 邮件预警系统
功能：实时监控鸡蛋主力合约（JD），价格异动时发送邮件提醒
作者：WorkBuddy
日期：2026-03-17
"""
...
# 收件人列表（可以是多个）
RECEIVERS = _config["smtp"]["receivers"]["jd"]

# 是否启用邮件发送（如果邮件配置有问题，先设为False）
ENABLE_EMAIL = True

# 监控合约（鸡蛋主力）
# 鸡蛋合约格式：DCE.jd2604（4月交割）、DCE.jd2605（5月交割）等
QUOTE_SYMBOL = "KQ.m@DCE.jd"  # 主力合约
SYMBOL = QUOTE_SYMBOL  # TQSDK可以直接用主力合约代码获取K线

# 预警配置
ALERT_CONFIG = {
    # 涨跌幅预警（基于昨结算价）
    "change_pct_threshold": 0.02,  # 2%
...
class EmailNotifier:
    ...
    def send_alert(self, title, content):
        ...
        try:
            ...
            # 构建邮件
            msg = MIMEMultipart()
            msg['From'] = SMTP_USERNAME  # From头部使用纯邮箱地址
            msg['To'] = ', '.join(RECEIVERS)
            msg['Subject'] = f"【鸡蛋期货】{title}"
...
class JDQuoteMonitor:
    ...
    def __init__(self):
        ...
        self.last_volume = None  # 上一次成交量
        self.volume_history = []  # 成交量历史（用于计算均量）
        
        # 快速变动检测
        self.price_history = []  # [(时间, 价格), ...]
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                # 2. 成交量异动（已禁用）
                # if self.last_volume is not None and self.quote.volume > self.last_volume:
                #     volume_change = self.quote.volume - self.last_volume
                #     alert = self.check_volume_alert(volume_change)
                #     if alert:
                #         alerts.append(alert)
